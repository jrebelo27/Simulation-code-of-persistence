          ...
          for (each_initial_diffusion_rate in c(0.2, 2)){
            ...
            for (each_row in 1:3){
              ...
              write.table(persistents_detoxified_area_generation_final_path ,
                          file = paste0("amount_susceptibles_detoxified_area_generation_", plasmid, "_", each_density, "_tau_zero", as.character(tau_zero), "_k1", as.character(k1), "_k3", as.character(k3), "_radius_", incrementing_radius, "_", incrementing_radius*number_conditions, "_L", each_row, ".csv"),
                          sep = ",",
                          row.names = FALSE,
                          col.names = TRUE)
